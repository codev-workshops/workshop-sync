...
async function main() {
  ...
  let target = targetInfos.find((t) => t.type === 'page' && sheetId && t.url.includes(sheetId));
  if (!target) {
  ...
  for (let i = 0; i < 40; i++) {
    if (await evaluate(`!!document.querySelector('#t-name-box') && document.querySelectorAll('.docs-sheet-tab').length > 0`)) break;
    await sleep(500);
  }

  // The tab element's textContent carries extra text, so match by suffix.
  const clicked = await evaluate(`(() => {
    const el = [...document.querySelectorAll('.docs-sheet-tab')].find(e => e.textContent.trim().endsWith(${JSON.stringify(tab)}));
    if (!el) return false;
  ...
  const nb = await evaluate(`(() => { const r = document.querySelector('#t-name-box').getBoundingClientRect(); return [r.x + r.width / 2, r.y + r.height / 2]; })()`);
...
async function key(page, keyName, code, modifiers = 0) {
  const base = { key: keyName, code: keyName.length === 1 ? `Key${keyName.toUpperCase()}` : keyName, windowsVirtualKeyCode: code, modifiers };
  await page('Input.dispatchKeyEvent', { type: 'keyDown', ...base, text: modifiers ? undefined : keyName.length === 1 ? keyName : undefined });
